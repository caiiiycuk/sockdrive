...
dir.iterator = function(dirChain, opts) {
    ...
    function getSectorBuffer(n, cb) {
        if (cache.n === n) cb(null, cache.buffer);
            ...
            cache.n = cache.buffer = null, dirChain.readSectors(n, _.allocBuffer(dirChain.sectorSize), function(e, d) {
                if (e) cb(e);
    ...
    function getNextEntry(cb) {
        ...
        getSectorBuffer(secIdx, function(e, buf) {
            ...
            const entryType = (attrByte === S.longDirFlag) ? S.longDirEntry : S.dirEntry_simple;
...
dir.makeStat = function(vol, entry) {
    ...
    function extractDate(prefix) {
        ...
        const time = entry[prefix+"Time"] || { hours: 0, minutes: 0, seconds_2: 0 };
...
dir.init = function(vol, dirInfo, cb) {
    ...
    const initialCluster = _.allocBuffer(dirChain.sectorSize*vol._sectorsPerCluster);
...
dir.addFile = function(vol, dirChain, entryInfo, opts, cb) {
    ...
    if (1 || mainEntry.Name._lossy) { // HACK: always write long names until `._lossy` is more useful!
        ...
        const S_lde_f = S.longDirEntry.fields;
        const ENTRY_CHUNK_LEN = (S_lde_f.Name1.size + S_lde_f.Name2.size + S_lde_f.Name3.size)/2;
    ...
    if (entryInfo.tail) {
        ...
        const sufIdx = (~endIdx) ? Math.min(endIdx, name.length-suffix.length) : name.length-suffix.length;
    ...
    vol.allocateInFAT(dirChain.firstCluster || 2, function(e, fileCluster) {
        ...
        const nameBuf = S.dirEntry.fields["Name"].bytesFromValue(mainEntry.Name);
